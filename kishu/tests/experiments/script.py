...
def run_test(notebook_path, cell_num_to_restore: int, enable_incremental_store):
    ...
    # Get notebook namespace contents at cell execution X and contents after checking out cell execution X.
        ...
        notebook.execute_full_checkout_branch_test(cell_num_to_restore)
...
if __name__ == "__main__":
    for nb, cell in [
        ...
        ('QiskitDemo_NCSA_May2023.ipynb', 10),
        ('store-sales-ts-forecasting-a-comprehensive-guide.ipynb', 25),
        ('sklearn_tweet_classification.ipynb', 32),
        ('tps-mar-fast-workflow-using-scikit-learn-intelex.ipynb', 34),
        ('bruteforce-clustering.ipynb', 11),
        ('Overview_of_Ray.ipynb', 9),
        ('pytorch-resnet34-96-6-accuracy.ipynb', 23)
    ]:
        default_n_threads = 8
        os.environ['OPENBLAS_NUM_THREADS'] = f"{default_n_threads}"
        os.environ['MKL_NUM_THREADS'] = f"{default_n_threads}"
