...
def test_create_variable_snapshot():
    ...
    assert len(self.graph.variable_snapshots["y"]) == 1
...
def test_add_cell_execution():
    ...
    assert len(self.graph.cell_executions) == 1
...
def test_update_graph(self):
    ...
    assert vs1.input_ces[0] == self.graph.cell_executions[0]
    assert len(self.graph.cell_executions[0].src_vss) == 1
    assert len(self.graph.cell_executions[0].dst_vss) == 3
