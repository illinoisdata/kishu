...
from kishu.storage.config import PersistentConfig
...
class KishuCommand:
    ...
    def _join_selected_commit(
        ...
        if PersistentConfig(KishuPath.database_path(notebook_path)).get("PLANNER", "incremental_store", True):
            temp_ahg = AHG(KishuDiskAHG(KishuPath.database_path(notebook_path)))
            temp_graph = KishuCommitGraph.new_var_graph(KishuPath.database_path(notebook_path))

            restore_plan = CheckpointRestorePlanner._generate_incremental_restore_plan(
                KishuPath.database_path(notebook_path),
                temp_ahg,
                temp_graph,
                Namespace(),
                temp_ahg.get_active_variable_snapshots(commit_entry.commit_id),
                set(),
