...
data_collect_log_dir = "/home/hanxif2/exp/kishu/llm_benchmark/"
...
# num_variables = []
# for i in range(7,10):
#     agent = NBGroupExecAgent(f"{group_prefix}_{i}", "non_kishu_start_over", 100)
...
#         e2e_time, final_run_steps = agent.e2e_execute(log_file,data_collect_log)
#         num_variable = agent.num_variables()
...
num_variables = []
for i in range(0,10):
    agent = NBGroupExecAgent(f"{group_prefix}_{i}", "non_kishu_start_middle", 100)
...
# for i in range(2, 4):
#     agent = NBGroupExecAgent(f"{group_prefix}_{i}", "test_semantic_error", 100)
